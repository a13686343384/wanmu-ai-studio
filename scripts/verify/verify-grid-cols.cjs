...
(async () => {
  ...
  const info = await page.evaluate(() => {
    const grids = [...document.querySelectorAll(".grid")].filter((g) => g.querySelector('[aria-label="生成首帧图"], [aria-label="出图"]'));
    return grids.map((g) => ({ cols: getComputedStyle(g).gridTemplateColumns.split(" ").length, cls: g.className.slice(0, 80) }));
  });
